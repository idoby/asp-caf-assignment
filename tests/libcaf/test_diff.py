...
def test_diff_nested_trees(temp_repo: Repository) -> None:
    ...
    modified_names = sorted([m.record.name for m in modified])
    assert modified_names == ['dir1', 'dir2']
    
    modified_dict = {m.record.name: m for m in modified}
    
    assert len(modified_dict['dir1'].children) == 1
    assert modified_dict['dir1'].children[0].record.name == 'file_a.txt'
    assert isinstance(modified_dict['dir1'].children[0], ModifiedDiff)

    assert len(modified_dict['dir2'].children) == 2
    dir2_children_names = sorted([c.record.name for c in modified_dict['dir2'].children])
    assert dir2_children_names == ['file_b.txt', 'file_c.txt']
    
    dir2_children_dict = {c.record.name: c for c in modified_dict['dir2'].children}
    assert isinstance(dir2_children_dict['file_b.txt'], RemovedDiff)
    assert isinstance(dir2_children_dict['file_c.txt'], AddedDiff)
...
def test_diff_moved_file_added_first(temp_repo: Repository) -> None:
    ...
    assert len(modified_dict['dir1'].children) == 1
    dir1_child = modified_dict['dir1'].children[0]
    assert isinstance(dir1_child, MovedToDiff)
    assert dir1_child.record.name == 'file_a.txt'

    assert isinstance(dir1_child.moved_to, MovedFromDiff)
    ...
    assert len(modified_dict['dir2'].children) == 1
    dir2_child = modified_dict['dir2'].children[0]
    assert isinstance(dir2_child, MovedFromDiff)
    assert dir2_child.record.name == 'file_c.txt'

    assert isinstance(dir2_child.moved_from, MovedToDiff)
...
def test_diff_moved_file_removed_first(temp_repo: Repository) -> None:
    ...
    assert dir1_child.record.name == 'file_c.txt'
